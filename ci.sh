...
Patch_ksu() {
    ...
    #额外的修补
    grep -q CONFIG_KSU arch/arm64/configs/vendor/alpha_lao_com-perf_defconfig || \
        echo "CONFIG_KSU=y" >>arch/arm64/configs/vendor/alpha_lao_com-perf_defconfig
    grep -q CONFIG_OVERLAY_FS arch/arm64/configs/vendor/alpha_lao_com-perf_defconfig || \
        echo "CONFIG_OVERLAY_FS=y" >>arch/arm64/configs/vendor/alpha_lao_com-perf_defconfig
    #修补kernelsu/makefile
...
Releases() {
    ...
    md5tab=${md5:0:5}
    kernelversion=$(head -n 3 "${GITHUB_WORKSPACE}"/android_kernel_lge_sm8150-"${KERNEL_HASH}"/Makefile | awk '{print $3}' | tr -d '\n')
...
EOF
#llvm dc build
make -j"$(nproc --all)" O=out vendor/alpha_lao_com-perf_defconfig \
    ARCH=arm64 \
    SUBARCH=arm64 \
    LLVM=1
